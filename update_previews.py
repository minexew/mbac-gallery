...
def render_mbac(title, path, mbac, sha1, rel_output_path, is_thumb, resolution):
    ...
    if axis_forward is None: axis_forward = '-Z'
    if axis_up is None: axis_up = 'Y'

    output_path = workdir / rel_output_path

    if (record and record["version"] >= MIN_VERSION and record["texture_sha1"] == texture_sha1
            and record["axis_forward"] == axis_forward and record["axis_up"] == axis_up
    ...
    print(f"RENDER title={title} path={path} {resolution=} {is_thumb=} texture_path={texture_path} {axis_forward=} {axis_up=}")
    # TODO: just import the function
    subprocess.check_call(["tools/mbac2obj.py", "tmp.mbac", "vertexdump.obj"])
    render_obj("vertexdump.obj", "preview", texture=texture_path, resolution=resolution,
            axis_forward=axis_forward, axis_up=axis_up)
    os.rename("preview0000.png", output_path)

    previews_db.add_mbac_preview(sha1=sha1, thumb=is_thumb,
            filename=str(rel_output_path), width=resolution[0], height=resolution[1],
            version=VERSION, texture_sha1=texture_sha1,
            axis_forward=axis_forward, axis_up=axis_up,
            )
    ...
    with zipfile.ZipFile(path, mode='r') as z:
        for info in z.infolist():
            ...
            if ext == '.BMP' or ext == '.PNG':
                ...
                if (workdir / rel_thumbs_dir / (sha1 + ".png")).is_file() and (workdir / rel_full_dir / (sha1 + ".png")).is_file():
                    continue
...
for path in args.jars:
    ...
    with zipfile.ZipFile(path, mode='r') as z:
        for info in z.infolist():
            if args.resource and info.filename != args.resource: continue
            ...
            if ext == '.MBAC':
                ...
                if sha1 not in bad_resource_sha1s:
                    ...
                    render_mbac(title, info.filename, mbac, sha1, rel_thumbs_dir / (sha1 + ".png"),
    ...
    with zipfile.ZipFile(path, mode='r') as z:
        for info in z.infolist():
            ...
            if ext == '.MBAC':
                ...
                if sha1 not in bad_resource_sha1s:
                    mbac = fishlabs_obfuscation.normalize(mbac, ext)
                    render_mbac(title, info.filename, mbac, sha1, rel_full_dir / (sha1 + ".png"),
                            is_thumb=False, resolution=(1280, 720))
